...
contract DssSpellTest is GoerliDssSpellTestBase {
    // GOERLI ADDRESSES
    bytes32 constant ilk = "RWA008AT5-A";

    address rwaOperator = addr.addr("RWA008AT5_A_OPERATOR"); // TODO set the right address
    DSTokenAbstract rwagem = DSTokenAbstract(addr.addr("RWA008AT5"));
    GemJoinAbstract rwajoin = GemJoinAbstract(addr.addr("MCD_JOIN_RWA008AT5_A"));
    RwaLiquidationLike oracle = RwaLiquidationLike(addr.addr("MIP21_LIQUIDATION_ORACLE"));
    RwaUrnLike rwaurn = RwaUrnLike(addr.addr("RWA008AT5_A_URN"));
    RwaInputConduitLike rwaconduitin = RwaInputConduitLike(addr.addr("RWA008AT5_A_INPUT_CONDUIT"));
    RwaOutputConduitLike rwaconduitout = RwaOutputConduitLike(addr.addr("RWA008AT5_A_OUTPUT_CONDUIT"));
    ...
    function testCollateralIntegrations() private { // make public to use
        ...
        checkIlkIntegration(
            "RWA008AT5-A",
            GemJoinAbstract(addr.addr("MCD_JOIN_RWA008AT5_A")),
            ClipAbstract(addr.addr("MCD_CLIP_RWA008AT5_A")),
            addr.addr("PIP_RWA008AT5"),
            false,
            false,
            false
        );
    ...
    function testNewChainlogValues() public { // make public to use
        ...
        assertEq(chainLog.getAddress("RWA008AT5"),                  addr.addr("RWA008AT5"));
        assertEq(chainLog.getAddress("MCD_JOIN_RWA008AT5_A"),       addr.addr("MCD_JOIN_RWA008AT5_A"));
        assertEq(chainLog.getAddress("RWA008AT5_A_URN"),            addr.addr("RWA008AT5_A_URN"));
        assertEq(chainLog.getAddress("RWA008AT5_A_INPUT_CONDUIT"),  addr.addr("RWA008AT5_A_INPUT_CONDUIT"));
        assertEq(chainLog.getAddress("RWA008AT5_A_OUTPUT_CONDUIT"), addr.addr("RWA008AT5_A_OUTPUT_CONDUIT"));

        // TODO Change this to the right version
        assertEq(chainLog.version(), "0.3.4");
    }
    ...
    function testNewIlkRegistryValues() public { // make public to use
        ...
        assertEq(reg.pos("RWA008AT5-A"),    5);
        assertEq(reg.join("RWA008AT5-A"),   addr.addr("MCD_JOIN_RWA008AT5_A"));
        assertEq(reg.gem("RWA008AT5-A"),    addr.addr("RWA008AT5"));
        assertEq(reg.dec("RWA008AT5-A"),    DSTokenAbstract(addr.addr("RWA008AT5")).decimals());
        assertEq(reg.class("RWA008AT5-A"),  3); // Class 3 = Rwa Token
        // assertEq(reg.pip("RWA008AT5-A"),    addr.addr("PIP_RWA008AT5"));
        // We don't have auctions for this collateral
        // assertEq(reg.xlip("RWA008AT5-A"),   addr.addr("MCD_CLIP_RWA008AT5_A"));
        assertEq(reg.name("RWA008AT5-A"), "RWA008AT5-A: SG Forge OFH");
        assertEq(reg.symbol("RWA008AT5-A"), "RWA008AT5");
    }

    function testNewPermissions() public {
        address MCD_VAT = addr.addr("MCD_VAT");
        address MCD_JOIN_RWA008AT5_A = addr.addr("MCD_JOIN_RWA008AT5_A");
        address RWA008AT5_A_URN = addr.addr("RWA008AT5_A_URN");
        ...
        assertEq(WardsLike(MCD_VAT).wards(MCD_JOIN_RWA008AT5_A), 1);
        assertEq(CanLike(RWA008AT5_A_URN).can(RWA_URN_PROXY_ACTIONS), 1);
    }


    function testSpellIsCast_RWA008AT5_INTEGRATION_BUMP() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
        ...
        hevm.warp(castTime);
        (, address pip, , ) = oracle.ilks("RWA008AT5-A");
    ...
    function testSpellIsCast_RWA008AT5_INTEGRATION_TELL() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
        ...
        uint256 castTime = block.timestamp + pause.delay();
        hevm.warp(castTime);
        (, , , uint48 tocPre) = oracle.ilks("RWA008AT5-A");
        assertTrue(tocPre == 0);
        assertTrue(oracle.good("RWA008AT5-A"));
        tellSpell.cast();
        (, , , uint48 tocPost) = oracle.ilks("RWA008AT5-A");
    ...
    function testSpellIsCast_RWA008AT5_INTEGRATION_TELL_CURE_GOOD() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
    ...
    function testFailSpellIsCast_RWA008AT5_INTEGRATION_CURE() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA008AT5_INTEGRATION_TELL_CULL() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA008AT5_OPERATOR_GET_RWA008AT5_TOKEN() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA008AT5_OPERATOR_LOCK_DRAW_CONDUITS_WIPE_FREE() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast(address(spell));
            assertTrue(spell.done());
        ...
        (, uint256 rate, , , ) = vat.ilks("RWA008AT5-A");
        ...
        jug.drip("RWA008AT5-A");

        (, rate, , , ) = vat.ilks("RWA008AT5-A");
...
contract CullSpellAction {
    ...
    function execute() public {
        RwaLiquidationLike(CHANGELOG.getAddress("MIP21_LIQUIDATION_ORACLE")).cull(
            ilk,
            CHANGELOG.getAddress("RWA008AT5_A_URN")
        );
    }
}
